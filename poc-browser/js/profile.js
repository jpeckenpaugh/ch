...
import { extractCompanyInfo, extractCompanyWebsite } from "./company-info/extractor.js";
import { canonicalUrl } from "./news-ranking/normalize.js";
import {
  esc,
  completenessBadge,
  formatDate,
    ...
    <div class="card mb-4">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2 align-items-center">
          ...
          ${write ? `
          <button class="btn btn-sm btn-outline-primary" id="find-news-btn">
            <i class="bi bi-stars me-1"></i>Find news
          </button>
...
function filesCard() {
  return `<div class="card mb-4"><div class="card-header">Files</div><div class="card-body text-secondary">Files are not included in this browser workspace.</div></div>`;
}
...
function wireCompanyInfo(container, body, company, countries, write) {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      status.textContent = "Looking up the company website…";
      const websiteLookup = await extractCompanyWebsite({model, companyName: company.name});
      proposal.website = websiteLookup.website;
      ...
      result.querySelector("#apply-company-info").onclick = async event => {
        ...
        try {
          ...
          if (!company.references.some(reference => canonicalUrl(reference.url) === canonicalUrl(source.url))) {
            await createReference(company.id, {title: `Wikipedia: ${source.title}`, url: source.url, description: null});
          }
...
function wireNews(container, body, company, write) {
  ...
  const findNewsBtn = body.querySelector("#find-news-btn");
  if (findNewsBtn) findNewsBtn.addEventListener("click", () => openFinder());
